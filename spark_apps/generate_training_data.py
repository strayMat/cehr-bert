...
def main(
    ...
    visit_occurrence = visit_occurrence.select(
        "visit_occurrence_id", "visit_concept_id", "person_id"
    )
    person = preprocess_domain_table(spark, input_folder, PERSON)
    # eds-modified: birth_datetime already exists in the eds
    person = person.select("person_id", "birth_datetime")
    ...
    if create_visits_from_dates:
        patient_event = (
            patient_event.join(person, "person_id")
            .select(
                [
                    patient_event[fieldName]
                    for fieldName in patient_event.schema.fieldNames()
                ]
                + ["birth_datetime"]
            )
    ...
    if include_prolonged_stay:
        udf = F.when(
            ...
            F.coalesce(
                (F.datediff("visit_end_date", "visit_start_date") > 7).cast(
                    "int"
                ),
                F.lit(0),
            ),
        ...
        visit_occurrence = (
            ...
            .withColumn(
                ...
                F.max("prolonged_length_stay").over(W.partitionBy("person_id")),
            )
            .distinct()
        )
    ...
    sequence_data.write.mode("overwrite").parquet(
        os.path.join(output_folder, p.parquet_data_path)
    )
