...
def create_bi_lstm_model(
    max_seq_length, vocab_size, embedding_size, concept_embeddings
):
    age_of_visit_input = tf.keras.layers.Input(name="age", shape=(1,))

    age_batch_norm_layer = tf.keras.layers.BatchNormalization(
        name="age_batch_norm_layer"
    )

    normalized_index_age = age_batch_norm_layer(age_of_visit_input)

    concept_ids = tf.keras.layers.Input(
        shape=(max_seq_length,), dtype="int32", name="concept_ids"
    )
    ...
    model = Model(
        inputs=[concept_ids, age_of_visit_input],
        outputs=output,
        name="Vanilla_BI_LSTM",
    )
...
def create_vanilla_feed_forward_model(vanilla_bert_model_path):
    ...
    bert_inputs = [
        ...
        if "visit" not in i.name
        or ("visit_segment" in i.name or "visit_concept_order" in i.name)
    ]

    contextualized_embeddings, _ = vanilla_bert_model.get_layer(
        "encoder"
    ).output
    _, _, embedding_size = contextualized_embeddings.get_shape().as_list()

    mask_input = [
        i for i in bert_inputs if "mask" in i.name and "concept" not in i.name
    ][0]
    ...
    output_layer = tf.keras.layers.Dense(
        1, name="prediction", activation="sigmoid"
    )
...
def create_sliding_bert_model(
    model_path, max_seq_length, context_window, stride
):
    age_at_index_date = tf.keras.layers.Input(name="age", shape=(1,))
    ...
    mask = tf.keras.layers.Input(
        shape=(max_seq_length,), dtype="int32", name="mask"
    )
    ...
    conv_bert_output = convolution_bert_layer(
        [concept_ids, visit_segments, time_stamps, ages, mask]
    )

    next_input = tf.keras.layers.concatenate(
        [conv_bert_output, normalized_index_age]
    )
    ...
    output = output_layer(
        dropout_dense_layer(dense_layer(dropout_conv_layer(next_input)))
    )

    model_inputs = [concept_ids, visit_segments, time_stamps, ages, mask]
    ffd_bert_model = tf.keras.models.Model(
        inputs=model_inputs + [normalized_index_age], outputs=output
    )
...
def create_vanilla_bert_bi_lstm_model(
    ...
    if index_stay_chapters:
        index_stay_chapters_input = tf.keras.layers.Input(
            name="index_stay_chapters", shape=(22,)
        )
        index_stay_chapters_batch_norm_layer = (
            tf.keras.layers.BatchNormalization(
                name="index_stay_chapters_batch_norm_layer"
            )
        )
        normalized_index_stay_chapters = index_stay_chapters_batch_norm_layer(
            index_stay_chapters_input
        )
    ...
    if index_stay_chapters:
        next_input = tf.keras.layers.concatenate(
            [next_input, normalized_index_stay_chapters]
        )
    next_input = dropout_dense_layer(dense_layer(next_input))
...
def create_temporal_bert_bi_lstm_model(
    max_seq_length, temporal_bert_model_path
):
    ...
    _, _, embedding_size = (
        temporal_bert_model.get_layer("temporal_encoder")
        .output[0]
        .get_shape()
        .as_list()
    )
    contextualized_embeddings, _, _ = temporal_bert_model.get_layer(
        "temporal_encoder"
    ).output
    ...
    contextualized_embeddings = tf.math.multiply(
        contextualized_embeddings, mask_embeddings
    )
