...
def join_domain_tables(domain_tables):
    ...
    for domain_table in domain_tables:
        ...
        concept_id_field, date_field, table_domain_field = get_key_fields(
            domain_table
        )
        # standardize the output columns
        domain_table = domain_table.where(
            F.col(concept_id_field).cast("string") != "0"
...
def preprocess_domain_table(
    spark, input_folder, domain_table_name, with_rollup=False
):
    domain_table = spark.read.parquet(
        create_file_path(input_folder, domain_table_name)
    )
    ...
    if with_rollup:
        ...
        if (
            ...
            concept = spark.read.parquet(
                create_file_path(input_folder, "concept")
            )
            concept_ancestor = spark.read.parquet(
                create_file_path(input_folder, "concept_ancestor")
            )
            domain_table = roll_up_to_drug_ingredients(
                domain_table, concept, concept_ancestor
            )
        ...
        if (
            ...
            domain_table = roll_up_diagnosis(
                domain_table, concept, concept_relationship
            )
        ...
        if (
            ...
            domain_table = roll_up_procedure(
                domain_table, concept, concept_ancestor
            )
...
def roll_up_to_drug_ingredients(drug_exposure, concept, concept_ancestor):
    ...
    drug_ingredient = (
        ...
        .join(
            concept_ancestor,
            F.col("drug_concept_id") == F.col("descendant_concept_id"),
        )
    ...
    drug_ingredient_fields = [
        F.coalesce(
            F.col("ingredient_concept_id"), F.col("drug_concept_id")
    ...
    drug_exposure = drug_exposure.join(
        drug_ingredient, "drug_concept_id", "left_outer"
...
def roll_up_diagnosis(condition_occurrence, concept, concept_relationship):
    ...
    condition_icd = (
        ...
        .join(
            concept,
            (F.col("condition_source_concept_id") == F.col("concept_id")),
        )
    ...
    condition_occurrence_fields.append(
        F.coalesce(
            F.col("ancestor_concept_id"), F.col("condition_source_concept_id")
    ...
    condition_occurrence = (
        ...
        .withColumn(
            "condition_concept_id", F.col("condition_source_concept_id")
        )
    )
...
def roll_up_procedure(procedure_occurrence, concept, concept_ancestor):
    ...
    parent_code_udf = F.udf(
        lambda code: extract_parent_code(code), T.StringType()
    )
    ...
    procedure_code = (
        ...
        .distinct()
        .join(
            concept, F.col("procedure_source_concept_id") == F.col("concept_id")
        )
    ...
    cpt_hierarchy_level_1 = cpt_hierarchy_level_1.withColumn(
        ...
        F.col("source_concept_id"), F.col("isroot").alias("ancestor_concept_id")
    )
    ...
    cpt_hierarchy_rollup_all = cpt_hierarchy_level_1.union(
        cpt_hierarchy_level_0
    ).drop_duplicates()
    ...
    procedure_10pcs_map = (
        procedure_10pcs.join(
            ...
            & (
                procedure_10pcs["child_vocabulary_id"]
    ...
    procedure_hcpcs = procedure_code.where(
        F.col("child_vocabulary_id") == "HCPCS"
    )
    procedure_hcpcs_map = (
        procedure_hcpcs.withColumn(
            "ancestor_concept_id", F.col("source_concept_id")
        )
    ...
    procedure_occurrence_fields.append(
        F.coalesce(
            F.col("ancestor_concept_id"), F.col("procedure_source_concept_id")
        ).alias("procedure_source_concept_id")
    )
    ...
    procedure_occurrence = (
        ...
        .withColumn(
            "procedure_concept_id", F.col("procedure_source_concept_id")
        )
    )
...
def create_sequence_data(
    ...
    earliest_visit_date_udf = F.min("date_in_week").over(
        W.partitionBy("visit_occurrence_id")
    )
    ...
    if classic_bert_seq:
        ...
        visit_start_date_udf = F.first("date").over(
            W.partitionBy(
                "cohort_member_id", "person_id", "visit_occurrence_id"
            ).orderBy("date")
        )
        ...
        separator_events = (
            ...
            .withColumn(
                "prev_visit_occurrence_id", prev_visit_occurrence_id_udf
            )
    ...
    patient_grouped_events = (
        ...
        .agg(
            F.sort_array(F.collect_set("date_concept_id_period")).alias(
        ...
        .withColumn(
            "orders",
            F.col("date_concept_id_period.order").cast(
                T.ArrayType(T.IntegerType())
            ),
        )
        .withColumn("dates", F.col("date_concept_id_period.date_in_week"))
        .withColumn(
            "concept_ids", F.col("date_concept_id_period.standard_concept_id")
        )
        .withColumn(
            "visit_segments", F.col("date_concept_id_period.visit_segment")
        )
        .withColumn("ages", F.col("date_concept_id_period.age"))
        .withColumn(
            "visit_concept_orders",
            F.col("date_concept_id_period.visit_rank_order"),
        )
    )
    ...
    if include_visit_type:
        patient_grouped_events = patient_grouped_events.withColumn(
            "visit_concept_ids",
            F.col("date_concept_id_period.visit_concept_id"),
        )
...
def create_sequence_data_with_att(
    ...
    if date_filter:
        patient_event = patient_event.where(
            F.col("date").cast("date") >= date_filter
        )

    if create_visits_from_dates:
        # create date_diff column
        window = W.partitionBy("cohort_member_id", "person_id").orderBy("date")
        patient_event = patient_event.withColumn(
            "date_difference",
            F.datediff(
                patient_event.date, F.lag(patient_event.date, 1).over(window)
            ),
        )
        ...
        patient_event = patient_event.withColumn(
            "_flg",
            F.coalesce(F.when(F.col("date_difference") > 1, 1), F.lit(0)),
        )
        patient_event = patient_event.withColumn(
            "visit_occurrence_id", F.sum("_flg").over(window)
        )
    ...
    visit_start_events = (
        patient_event.withColumn("date", visit_start_date_udf)
    ...
    visit_end_events = (
        patient_event.withColumn("date", visit_end_date_udf)
    ...
    priority_udf = (
        F.when(F.col("standard_concept_id") == "VS", -1)
        .when(F.col("standard_concept_id") == "VE", 1)
        .otherwise(0)
    )
    # Convert Date to days since epoch
    days_since_epoch_udf = (
        F.unix_timestamp("date") / F.lit(24 * 60 * 60)
    ).cast("int")
    ...
    prev_days_since_epoch_udf = F.lag("days_since_epoch").over(
        W.partitionBy("cohort_member_id", "person_id").orderBy(
            "date", "priority", "visit_occurrence_id"
        )
    )
    # Compute the time difference between the current record and the previous record
    time_delta_udf = F.when(
        F.col("prev_days_since_epoch").isNull(), 0
    ).otherwise(F.col("days_since_epoch") - F.col("prev_days_since_epoch"))
    ...
    patient_event = (
        patient_event.union(visit_start_events)
        .union(visit_end_events)
    ...
    time_token_insertions = (
        patient_event.where('standard_concept_id = "VS"')
    ...
    if include_visit_type:
        time_token_insertions = time_token_insertions.withColumn(
            "visit_concept_id", F.lit(0)
        )

    unioned_distinct_tokens = patient_event.union(
        time_token_insertions
    ).distinct()
    ...
    patient_grouped_events = (
        unioned_distinct_tokens.withColumn("order", order_udf)
        .withColumn("data_for_sorting", F.struct(struct_columns))
        .groupBy("cohort_member_id", "person_id")
        .agg(
            F.sort_array(F.collect_set("data_for_sorting")).alias(
                "data_for_sorting"
            ),
            F.max("visit_rank_order").alias("num_of_visits"),
            F.count("standard_concept_id").alias("num_of_concepts"),
        ...
        .withColumn(
            "concept_ids", F.col("data_for_sorting.standard_concept_id")
        )
        .withColumn("visit_segments", F.col("data_for_sorting.visit_segment"))
        .withColumn("ages", F.col("data_for_sorting.age"))
        .withColumn(
            "visit_concept_orders", F.col("data_for_sorting.visit_rank_order")
        )
    )
    ...
    if include_visit_type:
        patient_grouped_events = patient_grouped_events.withColumn(
            "visit_concept_ids", F.col("data_for_sorting.visit_concept_id")
        )
...
def create_concept_frequency_data(patient_event, date_filter=None):
    ...
    take_concept_ids_udf = F.udf(
        lambda rows: [row[0] for row in rows], T.ArrayType(T.StringType())
    )
    take_freqs_udf = F.udf(
        lambda rows: [row[1] for row in rows], T.ArrayType(T.IntegerType())
    )

    num_of_visits_concepts = patient_event.groupBy(
        "cohort_member_id", "person_id"
    ).agg(
    ...
    patient_event = (
        patient_event.groupBy(
            "cohort_member_id", "person_id", "standard_concept_id"
        )
        .count()
...
def create_retain_data(patient_event):
    ...
    training_data = (
        ...
        .agg(
            F.collect_list(F.col("standard_concept_id").cast("string")).alias(
                "concept_ids"
            )
        )
        .groupby("person_id")
        .agg(F.collect_list("concept_ids").alias("visit_concept_ids"))
    )
    ...
    token_ids = tokenizer.texts_to_sequences(
        training_data_pd["visit_concept_ids"].explode()
    )
    tokenized_training_data_pd = pd.DataFrame(
        zip(person_ids, token_ids), columns=["person_id", "token_ids"]
    )
    retain_training_data = (
        tokenized_training_data_pd.groupby("person_id")["token_ids"]
        .apply(list)
        .reset_index()
    )
    ...
    for domain_table_name in domain_table_list:
        domain_tables.append(
            preprocess_domain_table(
                spark, input_folder, domain_table_name, with_rollup
            )
        )
    patient_ehr_records = join_domain_tables(domain_tables)
    patient_ehr_records = patient_ehr_records.where(
